...
class AppTestCase(unittest.TestCase):
    ...
    def test_timeline_page(self):
       ...
       assert '<input class="input" type="text" id="name" name="name">' in html
       assert '<label for="email">Email</label>' in html
       assert '<input class="input" type="text" id="email" name="email">' in html
    ...
    def post_sucess(self):
       response = self.client.post("/api/timeline_post", data={"name":"test", "email": "john@example.com", "content": "Hello world, I'm John!"})
       assert response.status_code == 200
       assert response.is_json
